...
**使用大量的小批量进行训练在训练速度和结果上都会更好。**原始的Bert模型使用batchsize=256的模型训练1M个step，这等价于batchsize=2K的模型训练125K个step，等价于batchsize=8K训练31K个step。结果如图：

![image-20220529201700824](https://raw.githubusercontent.com/Rien190/ImgURL/master/img/202205292017892.png)

更小的批量，能够能到更好的精确度。但是本文没有确定多大的batchsize最适合。
...
首先按照BERT-Large架构（L=24，H=1024，A=16355m）对RoBERTa进行训练。用BOOKCORPUS和WIKIPEDIA数据集进行了100K步预训练。使用1024块V100 GPU对模型进行了大约一天的预训练。
